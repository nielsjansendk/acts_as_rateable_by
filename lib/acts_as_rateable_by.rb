...
module Acts #:nodoc:
  module RateableBy #:nodoc:
    ...
    module ClassMethods
      def acts_as_rateable_by
        has_many :ratings, :as => :rateable, :dependent => :destroy
        include Acts::RateableBy::InstanceMethods
        extend Acts::RateableBy::SingletonMethods
      end
    end
    ...
    module SingletonMethods
      ...
      def find_user_ratings_for(user,obj)
        rateable = ActiveRecord::Base.send(:class_name_of_active_record_descendant, self).to_s

        Rating.find(:first,
                    :conditions => ["rateable_id = ? and rateable_type = ? and rater_type = ? and rater_id = ?", obj.id, rateable, "User", user.id]
                    )
      end
